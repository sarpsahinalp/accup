...
def modelfit(alg, dtrain, predictors, performCV=True, printFeatureImportance=True, cv_folds=5):
    ...
    alg.fit(dtrain[predictors], dtrain['Like'])
    ...
    if performCV:
        cv_score = cross_val_score(alg, dtrain[predictors], dtrain['Like'], cv=cv_folds,
    ...
    "\nModel Report"
    print
    "Accuracy : %.4g" % metrics.balanced_accuracy_score(dtrain['Like'].values, dtrain_predictions)
    print
    "AUC Score (Train): %f" % metrics.roc_auc_score(dtrain['Like'], dtrain_predprob)
...
predictors = train.drop(['Like'], axis=1).columns
